module BlueJay
	class TwitterParser < Parser
		...
		def self.parse_response(response, data)
			...
			(raise "BOOM"; return) if data.nil? || !data.kind_of?(Net::HTTPResponse)
      ...
      begin
				...
				#raise "GLASS: kinda? #{response.kind_of?(Net::HTTPSuccess)}, chalmers #{response.errors.nil?}, gorman: #{response.successful?}" unless response.successful?
				...
				#raise "VOOL: #{response.inspect}" unless response.successful?
			end
